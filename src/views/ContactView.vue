...
function trackContactClick(label: string) {
	posthog.capture('contact_link_clicked', { channel: label })
}
...
<template>
	<div class="shell shell--reading">
		...
		<header class="page-head">
			<h1>Get in touch.</h1>
			<p class="lede">
				If something here resonates, reach out. Always glad to start a conversation, no agenda
				needed.
			</p>
		</header>
		...
		<ul class="channels">
			<li v-for="c in channels" :key="c.label">
				<a
					...
					@click="trackContactClick(c.label)"
				>
...
.banner {
	...
	background-image:
		url('/img/contact-vignette-v2.webp'),
		linear-gradient(
			155deg,
