...
<template>
	...
	<div class="shell shell--reading" v-else-if="project">
		...
		<header class="head">
			<p class="eyebrow">
				<RouterLink v-if="thread" to="/" class="thread-tag">{{ thread.label }}</RouterLink>
				· {{ project.year }}
			</p>
			<h1>{{ project.name }}</h1>
				...
				<ul class="links">
					<li v-for="l in project.links" :key="l.href">
						<a
							...
							rel="noopener"
							class="link"
							@click="trackExternalLinkClick(l.label, l.href)"
							>{{ l.label }} ↗</a
						>
					</li>
				</ul>
		...
		<nav v-if="siblings.length" class="siblings" aria-label="Same thread">
			<p class="meta-label">More in {{ thread?.label ?? 'this thread' }}</p>
			<ul>
